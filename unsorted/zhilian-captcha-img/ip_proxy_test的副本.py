...
def get_res(url):
    ...
    proxy = 'https://219.138.58.147:'
    # print(proxy)
    try:
        r = requests.get(img_url, headers=headers, proxies=proxy)
        if r.status_code == 200:
            return r.content
...
proxies = {'https': 'https://219.138.58.20:3128', 'http': '115.46.68.167:80'}
...
img_url = 'http://my.zhaopin.com/myzhaopin/picturetimestamp.asp'

# for i in range(203,1101):
#     img_name = str(i) + '.png'
#     print('downloading :', img_name)
#     im = Image.open(BytesIO(get_img(img_url)))
